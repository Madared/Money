...
public static class FundsFactory {
    public static IFunds Create(decimal amount, Currency currency) => amount switch {
        < 0 => NegativeDecimal.Create(amount).Map(negative => new Debt(negative, currency)).Data,
        > 0 => PositiveDecimal.Create(amount).Map(positive => new Money(positive, currency)).Data,
    ...
    public static Result<INonPositiveFunds> CreateNonPositive(decimal amount, Currency currency) => amount switch {
        < 0 => NegativeDecimal.Create(amount).Map(negative => new Debt(negative, currency) as INonPositiveFunds),
    ...
    public static Result<INonNegativeFunds> CreateNonNegative(decimal amount, Currency currency) => amount switch {
        > 0 => PositiveDecimal.Create(amount).Map(positive => new Money(positive, currency) as INonNegativeFunds),
