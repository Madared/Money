...
public record PositiveDecimal : IPositiveDecimal
{
    ...
    {
        if (!decimal.IsPositive(amount)) throw new InvalidPositiveDecimal(amount);
        Amount = amount;
    }
    ...
    public static Result<PositiveDecimal> Create(decimal amount) => decimal.IsPositive(amount)
